...
%% 得到训练集图片路径
ReadList  = textread('E:\大创项目\最终\keyframe_extraction\texts\text_test.txt','%s','delimiter','\n');%载入用户输入关键字
for index = 1:group_num
    for i = 1:group_size
        path_index  =  path_index+1;
        name= strcat('.\image_data\',char(ReadList(index,:)),'\a (',int2str(i),').jpg');
