...
%%
% Proto 是映射矩阵（相当于source）
% X = Target_Projected_Data = Target_Data*Xt;
function [index,index2,index3] = cvKnn(X, Proto, ProtoClass, K, distFunc,weight)
...
for i=1:20
    minframe(i,1) = min(d(1:100,i));
    minframe(i,2) = min(d(101:200,i));
    minframe(i,3) = min(d(i));
end

[temp1,index]= sort(minframe(:,1));
index = index(1:10,:);
[temp1,index2]= sort(minframe(:,2));
index2 = index2(1:10,:);
[temp1,index3]= sort(minframe(:,3));
index3 = index3(1:10,:);
